...
def generate_random_mask(args, x, list_of_sentences, n_samples=1000, subspace_limit=0):
    ...
    if subspace_limit == 0:
        ...
        if n_samples == args.samples_min:
            ...
            column_index_every_row = [np.random.choice(sentence_num, num_of_zero, replace=False) for num_of_zero in num_of_zeros]
            ...
            mask_matrix = mask_matrix[np.where(mask_matrix.sum(axis=1) >= sentence_num-subspace_limit)[0], :].squeeze()
            mask_matrix = mask_matrix * 2 - 1
...
def mask_to_masked_sample(masks_tensor, sample_tensor, sentence_index_to_word_index, pad_idx=1):
    ...
    assert len(sentence_index_to_word_index) == masks_tensor.shape[1], f"{len(sentence_index_to_word_index)}, {masks_tensor.shape}"
    for each_mask in masks_tensor:
        word_mask = [each_mask[_i].repeat(word_num_in_every_sentence[_i]) for _i in range(len(sentence_index_to_word_index))]
...
for test_index in pbar:
    ...
    masked_samples_tensor = mask_to_masked_sample(masks_tensor, sample_tensor, sentence_index_to_word_index, pad_idx=PAD_IDX)
    ...
    for subspace_limit in [0, 1, 2, 4, 8, 16, 32]:
        ...
        masked_samples_tensor = mask_to_masked_sample(truthful_sample_masks, sample_tensor, sentence_index_to_word_index, pad_idx=PAD_IDX)
        masked_samples_tensor = masked_samples_tensor.long()
        ...
        scikit_lasso_result = (np.matmul(expanded_truthful_sample_basis, coef.reshape(-1, 1)) + LassoSolver.intercept_).reshape(-1)

        pbar.set_description("sentence length: %d, truthful value: %.4f" % (one_test_sample_length, np.mean(np.abs(scikit_lasso_result - model_truthful_values))))
...
os.makedirs("harmonica2degree", exist_ok=True)
for subspace_limit in [0, 1, 2, 4, 8, 16, 32]:
    if args.split_end - args.split_start == 0 and args.split_start == 0:
        np.save(f"harmonica2degree/harmonica2degree_final_lasso_output_subspace{subspace_limit}_seed{args.seed}",
                eval(f"final_lasso_output_{subspace_limit}"))
        np.save(f"harmonica2degree/harmonica2degree_final_model_output_subspace{subspace_limit}_seed{args.seed}",
                eval(f"final_model_output_{subspace_limit}"))
    else:
        np.save(
            f"harmonica2degree/harmonica2degree_final_lasso_output_subspace{subspace_limit}_seed{args.seed}_{args.split_start}_{args.split_end}",
            eval(f"final_lasso_output_{subspace_limit}"))
        np.save(
            f"harmonica2degree/harmonica2degree_final_model_output_subspace{subspace_limit}_seed{args.seed}_{args.split_start}_{args.split_end}",
            eval(f"final_model_output_{subspace_limit}"))
...
for C in C_range:
    for subspace_limit in [0, 1, 2, 4, 8, 16, 32]:
        if args.split_end - args.split_start == 0 and args.split_start == 0:
            np.save(
                f"harmonica2degree/harmonica2degree_truthful_lasso_output_C_{C}_subspace_{subspace_limit}_seed{args.seed}",
                eval(f"final_truthful_lasso_C_{C}_subspace_{subspace_limit}"))
            np.save(
                f"harmonica2degree/harmonica2degree_truthful_model_output_C_{C}_subspace_{subspace_limit}_seed{args.seed}",
                eval(f"final_truthful_model_C_{C}_subspace_{subspace_limit}"))
            np.save(
                f"harmonica2degree/harmonica2degree_truthful_sigma_C_{C}_subspace_{subspace_limit}_seed{args.seed}",
                eval(f"final_truthful_sigma_C_{C}_subspace_{subspace_limit}"))
            np.save(
                f"harmonica2degree/harmonica2degree_truthful_answer_C_{C}_subspace_{subspace_limit}_seed{args.seed}",
                eval(f"final_truthful_answer_C_{C}_subspace_{subspace_limit}"))
        else:
            np.save(
                f"harmonica2degree/harmonica2degree_truthful_lasso_output_C_{C}_subspace_{subspace_limit}_seed{args.seed}_{args.split_start}_{args.split_end}",
                eval(f"final_truthful_lasso_C_{C}_subspace_{subspace_limit}"))
            np.save(
                f"harmonica2degree/harmonica2degree_truthful_model_output_C_{C}_subspace_{subspace_limit}_seed{args.seed}_{args.split_start}_{args.split_end}",
                eval(f"final_truthful_model_C_{C}_subspace_{subspace_limit}"))
            np.save(
                f"harmonica2degree/harmonica2degree_truthful_sigma_C_{C}_subspace_{subspace_limit}_seed{args.seed}_{args.split_start}_{args.split_end}",
                eval(f"final_truthful_sigma_C_{C}_subspace_{subspace_limit}"))
            np.save(
                f"harmonica2degree/harmonica2degree_truthful_answer_C_{C}_subspace_{subspace_limit}_seed{args.seed}_{args.split_start}_{args.split_end}",
                eval(f"final_truthful_answer_C_{C}_subspace_{subspace_limit}"))
