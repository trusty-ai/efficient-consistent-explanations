...
def generate_random_mask(args, x, list_of_sentences, n_samples=1000, subspace_limit=0, anchor=None):
...
def generate_local_mask(args, x, list_of_sentences, n_samples=1000, subspace_limit=0):
    # should return 1 and -1
    assert x.shape[0] == 1  # default: batch size
    length = x.shape[1]
    sentence_num = len(list_of_sentences)
...
def generate_random_anchor(n_minus_1, x, list_of_sentences):
    assert x.shape[0] == 1  # default: batch size
    sentence_num = len(list_of_sentences)
    anchor_matrix = ((np.random.rand(n_minus_1, sentence_num) > .5) * 2 - 1).astype(int)
    anchor_matrix = np.vstack([np.ones([1, sentence_num]), anchor_matrix])
...
for test_index in pbar:
    ...
    anchor = generate_random_anchor(args.n - 1,
                                    text_list_to_token_tensor(one_test_sample, length=len(one_test_sample)),
                                    list_of_sentences
                                    )
    ...
    print(len(Counter(anchor_index_list)))
    ...
    for _anchor_i in used_n:
        ...
        print(_anchor_expanded_basis.shape, _anchor_values.shape)
        ...
        print("np.sum(_anchor_coef)", np.sum(_anchor_coef))
        anchor_params[_anchor_i] = _anchor_coef
        anchor_intercepts[_anchor_i] = LassoSolver.intercept_
    print("anchor_params sum", np.sum(anchor_params))
    ...
    for subspace_limit in [0, 1, 2, 4, 8, 16, 32]:
        ...
        truthful_sample_basis = generate_random_mask(args,
                                                     ...
                                                     n_samples=n_samples,
                                                     subspace_limit=args.subspace_limit)  # 1s and -1s
        truthful_sample_anchor_index_list = assign_basis_to_anchor(truthful_sample_basis, anchor,
                                                                   limit_set=used_n)  # np.array
        ...
        expanded_truthful_sample_basis = np.hstack(
            [np.ones((expanded_truthful_sample_basis.shape[0], 1)), expanded_truthful_sample_basis])
    ...
    print("start to calculate truthful")
...
for subspace_limit in [0, 1, 2, 4, 8, 16, 32]:
    if args.split_end - args.split_start == 0 and args.split_start == 0:
        np.save(
            f"harmonica2degreepartial_preciselasso_sample_sample{args.samples_min}_anchor{args.n}_consisloss{args.ep_consistent_loss}/harmonica2degreepartial_final_lasso_output_subspace{subspace_limit}_seed{args.seed}",
            eval(f"final_lasso_output_{subspace_limit}"))
        np.save(
            f"harmonica2degreepartial_preciselasso_sample_sample{args.samples_min}_anchor{args.n}_consisloss{args.ep_consistent_loss}/harmonica2degreepartial_final_model_output_subspace{subspace_limit}_seed{args.seed}",
