...
for test_index in pbar:
    ...
    for subspace_limit in [0, 1, 2, 4, 8, 16, 32]:
        ...
        # torch_lasso_result = (np.matmul(expanded_truthful_sample_basis, ridge_model.coef_.cpu().numpy().reshape(-1, 1)) + ridge_model.w[0].detach().cpu().numpy()).reshape(-1)
        scikit_lasso_result = (
        ...
        # q = 1
        # print("torch sgd lasso truthful value", np.mean(np.abs(torch_lasso_result - model_truthful_values)))
        # print("scikit-learn coordinate descent truthful value", np.mean(np.abs(scikit_lasso_result - model_truthful_values)))
...
os.makedirs("harmonica3degree", exist_ok=True)
for subspace_limit in [0, 1, 2, 4, 8, 16, 32]:
    if args.split_end - args.split_start == 0 and args.split_start == 0:
        np.save(f"harmonica3degree/harmonica3degree_final_lasso_output_subspace{subspace_limit}_seed{args.seed}",
                eval(f"final_lasso_output_{subspace_limit}"))
        np.save(f"harmonica3degree/harmonica3degree_final_model_output_subspace{subspace_limit}_seed{args.seed}",
                eval(f"final_model_output_{subspace_limit}"))
    else:
        np.save(
            f"harmonica3degree/harmonica3degree_final_lasso_output_subspace{subspace_limit}_seed{args.seed}_{args.split_start}_{args.split_end}",
            eval(f"final_lasso_output_{subspace_limit}"))
        np.save(
            f"harmonica3degree/harmonica3degree_final_model_output_subspace{subspace_limit}_seed{args.seed}_{args.split_start}_{args.split_end}",
            eval(f"final_model_output_{subspace_limit}"))

for C in C_range:
    for subspace_limit in [0, 1, 2, 4, 8, 16, 32]:
        if args.split_end - args.split_start == 0 and args.split_start == 0:
            np.save(f"harmonica3degree/harmonica3degree_truthful_lasso_output_C_{C}_subspace_{subspace_limit}_seed{args.seed}",
                    eval(f"final_truthful_lasso_C_{C}_subspace_{subspace_limit}"))
            np.save(f"harmonica3degree/harmonica3degree_truthful_model_output_C_{C}_subspace_{subspace_limit}_seed{args.seed}",
                    eval(f"final_truthful_model_C_{C}_subspace_{subspace_limit}"))
            np.save(f"harmonica3degree/harmonica3degree_truthful_sigma_C_{C}_subspace_{subspace_limit}_seed{args.seed}",
                    eval(f"final_truthful_sigma_C_{C}_subspace_{subspace_limit}"))
            np.save(f"harmonica3degree/harmonica3degree_truthful_answer_C_{C}_subspace_{subspace_limit}_seed{args.seed}",
                    eval(f"final_truthful_answer_C_{C}_subspace_{subspace_limit}"))
        else:
            np.save(f"harmonica3degree/harmonica3degree_truthful_lasso_output_C_{C}_subspace_{subspace_limit}_seed{args.seed}_{args.split_start}_{args.split_end}",
                eval(f"final_truthful_lasso_C_{C}_subspace_{subspace_limit}"))
            np.save(f"harmonica3degree/harmonica3degree_truthful_model_output_C_{C}_subspace_{subspace_limit}_seed{args.seed}_{args.split_start}_{args.split_end}",
                eval(f"final_truthful_model_C_{C}_subspace_{subspace_limit}"))
            np.save(f"harmonica3degree/harmonica3degree_truthful_sigma_C_{C}_subspace_{subspace_limit}_seed{args.seed}_{args.split_start}_{args.split_end}",
                    eval(f"final_truthful_sigma_C_{C}_subspace_{subspace_limit}"))
            np.save(f"harmonica3degree/harmonica3degree_truthful_answer_C_{C}_subspace_{subspace_limit}_seed{args.seed}_{args.split_start}_{args.split_end}",
                eval(f"final_truthful_answer_C_{C}_subspace_{subspace_limit}"))
